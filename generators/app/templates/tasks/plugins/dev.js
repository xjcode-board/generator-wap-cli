...
module.exports = function(config) {
  var html = [
    ...
    tmpHtml = '',
    length = '/dev/pages/'.length,
    collector = listdir('./dev/pages'),
  ...
  function listdir(dir) {
    ...
    files.forEach(function(file) {
      ...
      var url = absolutePath.substring(
        absolutePath.indexOf('./dev/pages/') + length + 1
      )
  ...
  function showdir(collector, level) {
    ...
    if (collector['type'] == 'file') {
      if (path.extname(file) === '.html' && basename !== 'pages.html') {
        if (level === 1) {
          tmpHtml +=
  ...
  html +=
    ...
    '");document.title= "' +
    config.projectName +
    ' 资源列表";  $(".level1").prependTo(".table-body"); $(".td-qrcode i").bind("mouseenter ",function(){$("#qrcode").show().empty();new QRCode(document.getElementById("qrcode"), encodeURI(url.split("pages.html")[0]+$(this).parent().parent().find("a").attr("href")));});$("body").bind("click",function(){$("#qrcode").hide();});});</script></body></html>'

  var out = fs.createWriteStream('./dev/pages/pages.html', { encoding: 'utf8' })
