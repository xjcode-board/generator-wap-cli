...
var utils = {
  ...
  first: function(obj) {
    ...
    return obj.firstChild.nodeType == 1
      ? obj.firstChild
      : this.next(obj.firstChild)
  },
  /**
   * [last 最后的DOM节点]
   * @param  {[node]} obj [node节点]
  ...
  last: function(obj) {
    ...
    return obj.lastChild.nodeType == 1
      ? obj.lastChild
      : this.prev(obj.lastChild)
  },
  /**
   * [prev 前一个DOM节点]
   * @param  {[node]} obj [node节点]
  ...
  next: function(obj) {
    ...
    return obj.nextSibling.nodeType == 1
      ? obj.nextSibling
      : this.next(obj.nextSibling)
  },
  /**
   * [bind 事件绑定]
   * @param  {[node]}   obj    [node节点]
  ...
  getWebUrl: function(insName) {
    // 定义环境 dev-开发环境  stg-测试环境  rep-预发布   prd-生成环境
    if (this.env === 'dev') {
      return '/api/' + insName
    } else if (this.env === 'stg') {
      return 'http://stg.api.com/' + insName
    } else if (this.env === 'rep') {
      return 'http://api.com:9090/' + insName
    } else {
      return 'http://api.com:9090/' + insName
    }
  },
